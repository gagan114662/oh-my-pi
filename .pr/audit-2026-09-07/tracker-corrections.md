...
PR #59 already contains the trigger change; PR #58 is a blocked report, not an implementation. Preserve push branches main/omp2 and unfiltered PR triggers. Before requiring all five contexts, fix baseline #60 and license-policy failures: unused terminfo exception AND missing package-scoped uluru 3.1.0 MPL allowance. Prepared changes also add .config/docs/tools push coverage, run P9/P10/tool_sources, enforce model lints/current ADR paths, and verify regenerated dependency notices. No branch-protection settings have been changed. Do not claim every context is green while runtime-spec metadata and runtime test validation remain outstanding.
...
The observed run 34149276596 has three failing gates: format, runtime_spec and licenses; both proof jobs are consequently skipped. Format drift is corrected in the prepared patch. The spec panic came from deleted crates/app/src/envd/server.rs, with additional stale settings/telemetry/policy names. Those paths and canonical sv_interrupt_grace metadata are repaired, but static inspection finds 36 unregistered Python CONTROL literals plus 12 environment literals needing semantic triage (some are sentinels). Do not add guessed metadata or suppress these failures. License failures are both terminfo and uluru; the pinned cargo-deny check now passes locally. Full CI remains unverified; run the repaired spec gate and provisioned Rust suites before closure.
