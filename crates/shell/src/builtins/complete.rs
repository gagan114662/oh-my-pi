...
mod tests {
	...
	#[test]
	fn complete_accepts_full_action_and_special_spec_surface() {
		let command = CompleteCommand::new(strings(&[
			"complete",
			"-D",
			"-o",
			"nospace",
		...
		.unwrap();
		assert!(command.use_as_default);
		assert!(command.common_args.has_completion_spec());
	}

	#[test]
	fn compgen_and_compopt_accept_their_special_options() {
		assert!(
			CompGenCommand::new(strings(&["compgen", "-A", "command", "-W", "a b", "a"])).is_ok()
		);
		assert!(
			CompOptCommand::new(strings(&[
				"compopt", "-D", "-E", "-I", "-o", "nospace", "+o", "default",
			]))
			.is_ok()
		);
	}
