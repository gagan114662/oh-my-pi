...
pub(crate) async fn eval_extended_test_expr(
	...
	match expr {
		...
		recursive => eval_recursive_extended_test_expr(recursive, shell, params).await,
	}
}

#[async_recursion::async_recursion]
async fn eval_recursive_extended_test_expr(
	expr: &ast::ExtendedTestExpr,
	shell: &mut Shell<impl extensions::ShellExtensions>,
	params: &ExecutionParameters,
) -> Result<bool, error::Error> {
...
#[expect(
	...
	reason = "shell unary predicate grammar maps directly to this match"
)]
...
#[expect(
	clippy::too_many_lines,
	reason = "shell binary predicate grammar maps directly to this match"
)]
async fn apply_binary_predicate(
	...
	match op {
		ast::BinaryPredicate::StringMatchesRegex => {
			...
			let captures = regex.matches(s.as_str())?;
			let (matches, captures) =
				captures.map_or_else(|| (false, Vec::new()), |captures| (true, captures));
		...
		ast::BinaryPredicate::StringContainsSubstring => {
			let value = expansion::basic_expand_word(shell, params, left).await?;
			let substring = expansion::basic_expand_word(shell, params, right).await?;

			if shell.options().print_commands_and_arguments {
				shell
					.trace_command(params, std::format!("[[ {value} {op} {substring} ]]"))
					.await;
			}
...
fn string_contains(value: &str, substring: &str, case_insensitive: bool) -> bool {
	...
	value
		...
		.any(|candidate| candidate.eq_ignore_ascii_case(substring.as_bytes()))
}
