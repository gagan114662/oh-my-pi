	...
	for item in &seq.0 {
		...
		match item {
			...
			KeySequenceItem::Byte(b) => {
				...
				// If this is a control or meta stroke, the modifier only applies to this one
				// byte, so we need to push the stroke and start fresh for subsequent bytes.
...
mod tests {
	...
	fn test_basic_shell_cmd_binding_parse() -> Result<()> {
		...
		assert_eq!(binding.seq.0, [KeySequenceItem::Control, KeySequenceItem::Byte(b'k')]);
	...
	fn test_basic_readline_func_binding_parse() -> Result<()> {
		...
		assert_eq!(binding.seq.0, [KeySequenceItem::Meta, KeySequenceItem::Byte(b'x')]);
		assert_eq!(binding.target, ReadlineTarget::Function("some-function".to_string()));
