...
pub enum ExtendedGlobKind {
	/// The `+` extended glob; matches one or more occurrences of the inner
	/// pattern.
	Plus,
	...
	Exclamation,
	/// The `?` extended glob; matches zero or one occurrence of the inner
	/// pattern.
	Question,
	/// The `*` extended glob; matches zero or more occurrences of the inner
	/// pattern.
	Star,
}
...
/// * `enable_extended_globbing` - Whether to enable extended globbing
///   (extglob).
...
/// Returns whether or not a given character needs to be escaped in a regular
/// expression.
///
/// # Arguments
///
...
mod tests {
	...
	fn test_extended_glob() -> Result<()> {
		assert_eq!(pattern_to_regex_translator::extended_glob_pattern("@(a|b)", true)?, "(a|b)");

		assert_eq!(pattern_to_regex_translator::extended_glob_pattern("@(|a)", true)?, "(|a)");

		assert_eq!(pattern_to_regex_translator::extended_glob_pattern("@(|)", true)?, "(|)");

		assert_eq!(pattern_to_regex_translator::extended_glob_body("ab|ac", true)?, vec!["ab", "ac"],);

		assert_eq!(pattern_to_regex_translator::extended_glob_pattern("*(ab|ac)", true)?, "(ab|ac)*");

		assert_eq!(pattern_to_regex_translator::extended_glob_body("", true)?, Vec::<String>::new(),);
