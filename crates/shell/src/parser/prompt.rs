...
pub enum PromptPiece {
	...
	DollarOrPound,
	/// Special marker indicating the end of a non-printing sequence of
	/// characters.
	...
	ShellVersion,
	/// Special marker indicating the start of a non-printing sequence of
	/// characters.
...
mod tests {
	...
	fn basic_prompt() -> Result<()> {
		assert_eq!(parse(r"\u@\h:\w$ ")?, &[
			PromptPiece::CurrentUser,
			PromptPiece::Literal("@".to_owned()),
			PromptPiece::Hostname { only_up_to_first_dot: true },
			PromptPiece::Literal(":".to_owned()),
			PromptPiece::CurrentWorkingDirectory { tilde_replaced: true, basename: false },
			PromptPiece::Literal("$ ".to_owned()),
		]);
