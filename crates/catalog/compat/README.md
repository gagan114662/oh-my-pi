...
```
`variant-family` declares one reviewed provider-scoped collapsed family: positional provider and logical id, `name=` display name, optional `revision=` constraint for a `{rev}` template, and a body of `members "a" "b" …` (wire ids in priority order), `route "<tier>" "<wire-id>"` per effort tier (`off` included), and optional `mode`, `efforts`, `default-level`, `default-member`, `retired`, `budget "<tier>" "<tokens>"`, `requires-effort`, `suppress-when-off`, `no-thinking`, `preserve-absent-effort-routes`, and `aliases`. Every routed/member/retired/alias/default wire id in a template must carry `{rev}`; the compiler instantiates only revisions accepted by the constraint. `provider-alias` maps one provider-scoped selector spelling onto a logical id without making it a family member.
