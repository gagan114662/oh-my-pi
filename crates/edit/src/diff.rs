...
/// One byte-coordinate replacement derived from a line diff of `old` to `new`.
///
/// Coordinates index the old text.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct SpanEdit<'a> {
	/// Inclusive byte offset in the old text.
	pub start:       usize,
	/// Exclusive byte offset in the old text.
	pub end:         usize,
	/// Replacement slice from the new text.
	pub replacement: &'a str,
}

fn line_run_byte_len(text: &str, start: usize, count: u32) -> usize {
	text[start..]
		.split_inclusive('\n')
		.take(count as usize)
		.map(str::len)
...
/// Returns minimal sorted, non-overlapping byte edits turning `old` into `new`.
pub fn span_edits<'a>(old: &str, new: &'a str) -> Vec<SpanEdit<'a>> {
	...
	while index < runs.len() {
		let run = runs[index];
		if run.removed {
			let start = old_cursor;
			old_cursor += line_run_byte_len(old, old_cursor, run.count);
			if let Some(added) = runs.get(index + 1).filter(|next| next.added) {
				let replacement_start = new_cursor;
				new_cursor += line_run_byte_len(new, new_cursor, added.count);
				edits.push(SpanEdit {
					start,
					end: old_cursor,
					replacement: &new[replacement_start..new_cursor],
				});
				index += 2;
				continue;
			}
			edits.push(SpanEdit { start, end: old_cursor, replacement: &new[new_cursor..new_cursor] });
		} else if run.added {
			let replacement_start = new_cursor;
			new_cursor += line_run_byte_len(new, new_cursor, run.count);
			edits.push(SpanEdit {
				start:       old_cursor,
				end:         old_cursor,
				replacement: &new[replacement_start..new_cursor],
...
mod tests {
	...
	fn assert_span_edits_reproduce(old: &str, new: &str) {
		let edits = span_edits(old, new);
		let mut actual = old.to_owned();
		for edit in edits.iter().rev() {
			actual.replace_range(edit.start..edit.end, edit.replacement);
		}
		assert_eq!(actual, new);
	}

	#[test]
	fn span_edits_reproduce_line_changes() {
		assert_span_edits_reproduce("a\nb\nc\n", "a\nx\nc\n");
		assert_span_edits_reproduce("a\nb\n", "a\nb\nc");
		assert_span_edits_reproduce("a\nb\nc\n", "b\nc\n");
		assert!(span_edits("same\n", "same\n").is_empty());
	}
