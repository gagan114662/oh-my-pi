...
pub struct PathPolicy {
	...
	pub cwd:                  PathBuf,
	/// User home used to expand `~/` paths.
...
impl PathPolicy {
	...
	pub fn recover_missing(&self, authored: &str) -> Option<Resolved> {
		...
		let glob = omp_walker::CompiledWalkGlob::new([format!("**/{escaped}")]).ok()?;
		let started = Instant::now();
		let request = omp_walker::WalkRequest::new(&self.cwd)
			.hidden(true)
			.gitignore(true)
			.skip_git(true)
			...
			.cache(false)
			.limit(2)
			.filter(omp_walker::WalkFilter::all().glob(glob));
		let result = request
