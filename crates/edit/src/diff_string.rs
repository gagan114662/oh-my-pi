...
use omp_core::{FastHashSet, Str, StrMut};
use regex::Regex;
...
/// first observed in a later alignment.
#[derive(Clone, Default)]
pub struct PinnedMyersWindow {
	revision:      Option<Str>,
	text:          StrMut,
	rows:          FastHashSet<Str>,
	added_lines:   usize,
	removed_lines: usize,
}
...
impl PinnedMyersWindow {
	...
	pub fn advance(
		...
		let source = BlockContextSource { path: path.and_then(Path::to_str), lang: None };
		let diff = generate_diff_string(base, current, None, &source);
		if self.revision.as_deref() != Some(revision) {
			self.reset(Str::new(revision));
		}

		self.added_lines = diff.diff.lines().filter(|row| row.starts_with('+')).count();
		self.removed_lines = diff.diff.lines().filter(|row| row.starts_with('-')).count();
...
pub fn generate_diff_string(
	...
	let parts = crate::diff::line_changes_str(old, new);
...
pub fn generate_unified_diff_string(
	...
	let hunks = crate::diff::structured_patch_hunks_u16(
		&old_utf16,
		&new_utf16,
