...
//!
//! Threading and callbacks live in the host layer; this type is single
...
pub struct SessionConfig {
	/// Selected wire mode.
	pub mode:               EditMode,
	/// Path resolution and write policy.
	pub policy:             PathPolicy,
	/// Whether supported modes may recover with fuzzy matching.
	pub allow_fuzzy:        bool,
	/// Minimum accepted fuzzy-match confidence.
	pub fuzzy_threshold:    f64,
	/// Whether hashline anchors must have appeared in a read snapshot.
...
pub struct WriteRequest {
	...
	pub flush_lsp:    bool,
	/// Optional host LSP batch identifier.
...
pub trait EditWriter: Send + Sync {
	/// Persist one staged write and report the bytes that landed.
	fn write(&self, request: WriteRequest)
	-> impl Future<Output = EditResult<WriteResponse>> + Send;
}
...
pub struct ApplyRequest {
	/// Optional host LSP batch identifier.
	pub lsp_batch_id: Option<String>,
	/// Flush the LSP batch after the final write.
	pub lsp_flush:    bool,
}
...
pub struct FileOutcome {
	...
	pub text:               String,
	/// `before` parsed and `after` does not (`omp_ast` summary).
...
impl Session {
	/// Create a session with its mode configuration and persistent edit store.
	...
	/// Return this session's edit store.
	pub const fn store(&self) -> &EditStore {
		&self.store
	}

	/// Return this session's immutable configuration.
	pub const fn config(&self) -> &SessionConfig {
		&self.config
	}

	/// Return the selected mode engine.
	...
	/// Return whether the argument stream has finished.
...
pub fn source_parses(code: &str, path: &str) -> bool {
	...
	omp_ast::summary::summarize_code(omp_ast::summary::SummaryOptions {
		code:               code.to_owned(),
		lang:               None,
