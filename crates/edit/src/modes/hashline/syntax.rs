...
fn key(text: &str, path: &str, start: u32, end: u32) -> Key {
	(fast_hash64(text.as_bytes()), text.len(), path.to_owned(), start, end)
}
...
pub fn node_chain(lines: &[String], path: &str, line: u32) -> Vec<NodeSpan> {
	...
	if let Some(cached) = CACHE.lock().chains.get(&cache_key) {
		return cached.clone();
	}
...
pub fn enclosing_boundaries(lines: &[String], path: &str, start: u32, end: u32) -> Vec<u32> {
	...
	if let Some(cached) = CACHE.lock().boundaries.get(&cache_key) {
		return cached.clone();
	}
...
pub fn parses_cleanly(path: Option<&str>, text: &str) -> bool {
	...
	if let Some(cached) = CACHE.lock().parses.get(&cache_key) {
		return *cached;
	}
