...
fn build_line_map(previous: &str, current: &str) -> HashMap<u32, u32> {
	...
	for run in crate::diff::line_runs_str(previous, current) {
		if run.added {
...
/// Which retained snapshot chain a stale tag came from.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum RecoveryChain {
	/// Snapshot recorded from an externally observed document revision.
	External,
	/// Snapshot retained from an edit session.
	Session,
}

/// Remaps stale edits from `previous` onto `current` and applies them.
///
/// Returns `None` when recovery is not possible or would make no change.
pub fn recover_text(
	previous: &str,
	current: &str,
	edits: &[Edit],
	...
	let warning = match chain {
		RecoveryChain::External => RECOVERY_EXTERNAL_WARNING,
		RecoveryChain::Session => RECOVERY_SESSION_CHAIN_WARNING,
	};
	let Some((edits, offset)) = remap_edits(previous, current, edits) else {
		return Ok(None);
	};
	let Ok(applied) = apply_edits(current, &edits, ApplyOptions {
		clipboard,
		path,
