...
mod tests {
	...
	#[tokio::test]
	#[ignore = "requires system ffmpeg/ffprobe; explicitly executed by read-tail acceptance workflow"]
	async fn video_fixture_frame_time_and_preview() {
		let temporary = tempfile::tempdir().expect("fixture directory");
		// Apostrophe/semicolon characters verify argv handling without a shell.
		let path = temporary.path().join("demo';echo-not-executed.mp4");
		...
		let generation = generator
			.args([
				...
				"lavfi",
				"-i",
				"testsrc2=size=64x48:rate=2:duration=4",
				"-c:v",
				"libx264",
				"-threads",
				"1",
			...
			.status();
		assert!(
			tokio::time::timeout(Duration::from_secs(15), generation)
				.await
				.expect("generation deadline")
				.expect("ffmpeg installed")
				.success()
		);
		let frame = extract(&path, Selection::Frame(2))
			.await
			.expect("frame extraction");
		let time = extract(&path, Selection::Time(1.0))
			.await
			.expect("timestamp extraction");
		assert_eq!(frame.png, time.png, "frame 2 and timestamp 1s select the same source image");
		assert!(time.description.contains("Resolution: 64x48"));
		let preview = extract(&path, Selection::Preview)
			.await
			.expect("preview extraction");
		assert!(preview.description.contains("3x3"));
		assert_eq!(&preview.png[16..24], &[0, 0, 3, 192, 0, 0, 2, 28], "PNG IHDR is 960x540");
		assert!(matches!(extract(&path, Selection::Time(4.0)).await, Err(VideoFault::OutOfRange)));
		assert!(matches!(extract(&path, Selection::Frame(8)).await, Err(VideoFault::OutOfRange)));
		let corrupt = temporary.path().join("corrupt.mov");
		tokio::fs::write(&corrupt, b"not a video").await.unwrap();
		assert!(matches!(
			extract(&corrupt, Selection::Preview).await,
			Err(VideoFault::Process { .. })
		));
	}
}
